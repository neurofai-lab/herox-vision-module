...
except Exception:  # Allows direct source-tree execution without ROS package index.
    get_package_share_directory = None


PACKAGE_NAME = "hri_person_detect"
DEFAULT_MODEL_CFG = "model_cfg_latest_person_tiny.py"
DEFAULT_CHECKPOINT = "rtmdet-ins_tiny_8xb32-300e_coco_20221130_151727-ec670f7e.pth"


def _resolve_existing_path(user_value: Optional[str], package_subdir: str, filename: str) -> str:
    """
    Resolve model/config files in a way that works both from source and after colcon install.
    This replaces only hard-coded local file lookup. The detector logic remains unchanged.
    """
    candidates = []

    if user_value:
        candidates.append(os.path.abspath(os.path.expanduser(user_value)))

    # Current working directory, matching the original script behavior.
    candidates.append(os.path.abspath(filename))

    # Source-tree layout: repo/config and repo/models.
    here = os.path.dirname(os.path.realpath(__file__))
    repo_root = os.path.dirname(here)
    candidates.append(os.path.join(repo_root, package_subdir, filename))

    # Installed ROS share directory.
    if get_package_share_directory is not None:
        try:
            share_dir = get_package_share_directory(PACKAGE_NAME)
            candidates.append(os.path.join(share_dir, package_subdir, filename))
        except Exception:
            pass
    ...
    # Return the first explicit/default candidate so init_detector raises a useful error.
    return candidates[0]
...
class CameraSubscriber(Node):
    ...
    def __init__(self, distance_threshold, detection_confidence_score=0.5,
                 ...
                 model_cfg: Optional[str] = None, checkpoint: Optional[str] = None,
                 device: str = 'cuda:0', hri_track_timeout: float = 1.0):
        super().__init__('camera_subscriber')

        self.model_cfg = _resolve_existing_path(model_cfg, 'config', DEFAULT_MODEL_CFG)
        self.checkpoint = _resolve_existing_path(checkpoint, 'models', DEFAULT_CHECKPOINT)
        self.device = device
        self.hri_track_timeout = float(hri_track_timeout)
        ...
        # ROS4HRI-compatible body publishers.
        # Standard topics:
        #   /humans/bodies/tracked
        #   /humans/bodies/<body_id>/roi
        #   /humans/bodies/<body_id>/position
        self.hri_bodies_tracked_pub = self.create_publisher(IdsList, '/humans/bodies/tracked', 10)
        self.hri_roi_publishers = {}
        self.hri_position_publishers = {}
        self.hri_body_last_seen = {}
    ...
    def _make_hri_body_id(self, camera_name, track_id):
        """Return a stable ROS4HRI body/person-like ID derived from DeepSort track IDs."""
        cam_suffix = camera_name.replace("cam_", "c")
        raw_track_id = str(track_id).replace("/", "_").replace(" ", "_")
        return f"person_{cam_suffix}_{raw_track_id}"

    def _get_hri_body_publishers(self, body_id):
        if body_id not in self.hri_roi_publishers:
            self.hri_roi_publishers[body_id] = self.create_publisher(
                NormalizedRegionOfInterest2D, f"/humans/bodies/{body_id}/roi", 10
            )
        if body_id not in self.hri_position_publishers:
            self.hri_position_publishers[body_id] = self.create_publisher(
                PointStamped, f"/humans/bodies/{body_id}/position", 10
            )
        return self.hri_roi_publishers[body_id], self.hri_position_publishers[body_id]

    def _publish_hri_body_roi(self, body_id, x1, y1, x2, y2, confidence, image_shape, source_header, camera_name):
        h, w = image_shape[:2]
        roi = NormalizedRegionOfInterest2D()
        roi.header.stamp = source_header.stamp if source_header is not None else self.get_clock().now().to_msg()
        roi.header.frame_id = source_header.frame_id if source_header is not None and source_header.frame_id else camera_name
        roi.xmin = float(max(0.0, min(1.0, x1 / float(w))))
        roi.ymin = float(max(0.0, min(1.0, y1 / float(h))))
        roi.xmax = float(max(0.0, min(1.0, x2 / float(w))))
        roi.ymax = float(max(0.0, min(1.0, y2 / float(h))))
        roi.c = float(max(0.0, min(1.0, confidence)))
        roi_pub, _ = self._get_hri_body_publishers(body_id)
        roi_pub.publish(roi)

    def _publish_hri_body_position(self, body_id, coordinates_data, source_header, camera_name):
        pos = PointStamped()
        pos.header.stamp = source_header.stamp if source_header is not None else self.get_clock().now().to_msg()
        pos.header.frame_id = source_header.frame_id if source_header is not None and source_header.frame_id else camera_name
        pos.point.x = float(coordinates_data[0])
        pos.point.y = float(coordinates_data[1])
        pos.point.z = float(coordinates_data[2])
        _, pos_pub = self._get_hri_body_publishers(body_id)
        pos_pub.publish(pos)

    def _publish_hri_tracked_bodies(self):
        now = time.time()
        self.hri_body_last_seen = {
            body_id: last_seen
            for body_id, last_seen in self.hri_body_last_seen.items()
            if (now - last_seen) <= self.hri_track_timeout
        }
        msg = IdsList()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.ids = sorted(self.hri_body_last_seen.keys())
        self.hri_bodies_tracked_pub.publish(msg)
    ...
    def init_model(self):
        ...
        try:
            self.model = init_detector(self.model_cfg, self.checkpoint, device=self.device)
            self.get_logger().info(f"Loaded detector config: {self.model_cfg}")
            self.get_logger().info(f"Loaded detector checkpoint: {self.checkpoint}")
            self.get_logger().info(f"Detector device: {self.device}")
    ...
    def process_images(self, color_data, depth_data, camera_name):
        ...
        try:
            ...
            # # Save debug image only when person detections exist
            # if self.debug and debug_detections:
            ...
            for track in self.trackers[camera_name].update_tracks(
                ...
                if object_distance <= self.distance_threshold:
                    ...
                    object_distance = object_distance / 100  # mm -> m
                    ...
                    track_id = getattr(track, "track_id", len(bbox3d_array_msg.boxes))
                    body_id = self._make_hri_body_id(camera_name, track_id)
                    self.hri_body_last_seen[body_id] = time.time()
                    confidence = getattr(track, "det_conf", None)
                    if confidence is None:
                        confidence = 1.0
                    self._publish_hri_body_roi(
                        body_id, x1, y1, x2, y2, confidence, color_image.shape,
                        getattr(color_data, "header", None), camera_name
                    )
                    self._publish_hri_body_position(
                        body_id, coordinates_data, getattr(depth_data, "header", None), camera_name
                    )

            self._publish_hri_tracked_bodies()
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='Seconds to keep a ROS4HRI body ID tracked after the last detection.'
    )
    parser.add_argument(
        '--model_cfg',
        type=str,
        default=None,
        help='Optional path to the MMDetection config file.'
    )
    parser.add_argument(
        '--checkpoint',
        type=str,
        default=None,
        help='Optional path to the detector checkpoint file.'
    )
    parser.add_argument(
        '--device',
        type=str,
        default='cuda:0',
        help='Detector device, e.g. cuda:0 or cpu.'
    )
    ...
    camera_subscriber = CameraSubscriber(
        ...
        hri_track_timeout=args.hri_track_timeout,
